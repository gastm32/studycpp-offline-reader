...
前面的课程介绍了许多术语和概念，我们将在程序中使用它们。在本课中，将把这些知识集成到第一个简单程序中。
...
完成每个步骤时，将对应的代码手动输入（不要复制/粘贴）到编译器中，编译并运行。
...
由于需要将文本输出到控制台，并从用户的键盘获取输入，因此需要包含iostream来访问std::cout和std::cin。
...
首先，由于程序在进行最新更新之前可以正常编译，因此错误一定在刚才添加的代码中（第7行和第8行）。这大大减少了排查错误的工作量。第7行相当简单（只是一个变量定义），错误可能不在这里，因此第8行是最可能的罪魁祸首。
...
这是首选解决方案。当执行std::cout时，将计算表达式num\*2，结果是num的两倍。然后打印该值。num本身不会被更改，如果需要，稍后可以再次使用它。
...
当专注于弄清楚如何使程序工作时，甚至不确定某些代码能否正确运行。所以我们会暂时跳过错误处理和注释等内容。在代码中插入调试代码，以帮助诊断问题和发现错误。边做边学，认为可行的方案可能行不通，需要回过头来尝试另一种方法。

结果是，初始解决方案往往不够结构良好、健壮（防错）、可读或简洁。因此，即使程序开始工作，工作也没有完成（除非程序是一次性的）。下一步是清理代码，包括：删除（或注释掉）临时/调试代码、添加注释、处理错误情况、格式化代码，以及确保遵循最佳实践。即使如此，程序也可能不够优雅——也许存在可以合并的冗余逻辑、可以组合的多个语句、不需要的变量，或者其他可以简化的地方。

教程提供的解决方案中，很少有第一次编写就达到最优的。相反，它们是不断改进的结果，直到没有可改进之处为止。在许多情况下，读者仍然可以找到其他的改进建议！

所有这一切都是在说：如果你的解决方案没有一开始就完美无缺，不要沮丧。这很正常。编程中的完善是一个迭代过程（需要反复进行的过程）。

**注**

还有一件事：您可能会想，“C++有这么多规则和概念。我如何记住所有这些东西？”。

简短的回答：没有人能记住。使用C++分为两部分：一部分是运用你已经掌握的知识，另一部分是查找如何完成其余工作。

当你第一次阅读这个网站时，不要太注重记忆细节，而要更多地理解哪些功能是可用的。然后，当需要在编写的程序中实现某些功能时，可以回到这里（或到参考网站），重新了解具体的实现细节。
