...
它们非常直观。每个运算符的计算结果都是布尔值true（1）或false（0）。
...
这是多余的，因为==true实际上不会给条件语句增加任何额外的判断。相反，我们应该写：
...
不要添加不必要的 == 或 != 条件判断。这样做会使代码更难阅读，而且不会提供任何额外的价值。
...
如果在调试器中检查d1和d2的值，您可能会看到d1=0.010000000000005116和d2=0.99999999997868。这两个数字都接近0.01，但d1大于0.01，d2小于0.01。

使用任何关系运算符比较浮点值都可能是危险的。这是因为浮点值不精确，浮点运算中的舍入错误可能会导致它们比预期的稍小或稍大。
...
如果在操作数相似时偶尔得到错误答案是可以接受的，那么使用这些运算符也是可以的。这是一个取决于具体应用场景的判定。

例如，考虑一个游戏（如太空入侵者），您希望确定两个移动对象（如导弹和外星人）是否相交。如果对象仍然相距很远，则这些运算符将返回正确的答案。如果这两个对象非常接近，您可能会得到错误答案。在这种情况下，错误的答案可能根本不会被注意到（它只是看起来像是差一点就打中了），游戏将继续。
...
std::abs() 是头文件中的函数，它返回其参数的绝对值。因此，std::abs(a - b) <= absEpsilon 检查a和b之间的距离是否小于或等于传入的表示“足够近”的ε值。如果a和b足够近，函数返回true以指示它们相等。否则，它返回false。
...
这意味着每次调用该函数时，都必须选择一个适合输入的ε。既然已经知道必须根据输入的大小按比例缩放ε，不妨直接修改函数来自动完成这一步。
...
让我们更详细地研究一下这个看起来复杂的函数是如何工作的。在<=运算符的左侧，std::abs(a - b) 将a和b之间的距离作为正数告诉我们。
...
然而，在C++23之前，会遇到一个问题。在常量表达式中调用这两个constexpr函数，则它们将失败：
...
这是因为在常量表达式中使用的constexpr函数不能调用非constexpr函数，而std::abs直到C++23才成为constexpr。
