...
虽然关系（比较）运算符可以用于测试特定条件是真还是假，但它们一次只能测试一个条件。然而很多时候，我们需要同时判断多个条件是否成立。例如，要检查我们是否中了彩票，就必须逐一比较所选的每个号码是否与中奖号码匹配。在有6个数字的彩票中，这就涉及6次比较。还有些时候，我们需要判断多个条件中是否有任意一个为真。例如，今天不想上班的理由可能是生病了、太累了、或者中了彩票——只要其中任何一个条件成立即可。
...
逻辑OR运算符用于测试两个条件中是否有任意一个为真。如果左操作数的计算结果为true，或右操作数的计算结果为true，或两者都为true，则逻辑OR运算符返回true。否则返回false。
...
在这种情况下，我们使用逻辑OR运算符来测试左条件（value==0）或右条件（value==1）是否为真。如果其中任意一个为true（或两个都为true），则逻辑OR运算符的计算结果为true，这意味着执行if语句。如果两者都不为true，则逻辑OR运算符的计算结果为false，这意味着执行else语句。
...
为了使逻辑AND返回true，两个操作数的计算结果都必须为true。如果左操作数的计算结果为false，则无论右操作数的结果是true还是false，最终结果都必然为false。在这种情况下，逻辑AND运算符将立即返回false，而不再计算右边的操作数！这称为短路求值。

类似地，如果逻辑OR的左操作数为true，则整个OR条件的计算结果必为true。

短路求值是我们不应在复合表达式中使用带有副作用的运算符的又一个例证。请考虑以下代码片段：
...
逻辑OR和逻辑AND运算符是"操作数可以按任意顺序求值"这一规则的例外，因为标准明确规定左操作数必须首先求值。
...
因为逻辑AND和逻辑OR看起来像一对，所以许多程序员假设它们具有相同的优先级（就像加法与减法优先级相同、乘法与除法优先级相同一样）。然而，逻辑AND的优先级高于逻辑OR，因此逻辑AND运算符将在逻辑OR运算符之前计算（除非它们已被括号括起来）。

新手程序员通常会编写 value1 || value2 && value3 这样的表达式。由于逻辑AND具有更高的优先级，因此它的计算结果为 value1 ||（value2 && value3），而不是（value1 || value2）&& value3。如果假设从左到右执行（就像加法/减法或乘法/除法那样），将得到一个没有预料到的结果！
...
如果需要一种与非布尔操作数一起使用的逻辑异或形式，可以将操作数static\_cast为bool：
...
C++中的许多运算符（如运算符 || ）的名称只是符号。从历史上看，并不是所有的键盘和语言标准都支持输入这些符号。因此，C++提供了一组替代关键字，允许使用单词来代替符号。例如，可以使用关键字 or 来代替 ||。
