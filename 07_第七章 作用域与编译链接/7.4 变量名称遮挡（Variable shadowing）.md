...
每个代码块有自己独立的命名空间。那么，当我们在嵌套的内部块中有一个变量，该变量与外部块中的变量同名时，会发生什么呢？当这种情况发生时，嵌套块内的变量会“遮挡”外部变量。
...
因此，当我们将值10赋值给apples时，我们赋值的是嵌套块内的apples。打印该值（10）后，嵌套块结束，嵌套块中的apples被销毁。外围块中的apples的存在和值不受影响，我们通过打印apples（5）的值来证明这一点。
...
通常应避免局部变量的命名遮挡，因为这很容易导致意外使用或修改了错误的变量，从而引发难以排查的错误。当变量被遮挡时，某些编译器将发出警告。

出于相同原因，我们建议也避免遮挡全局变量。如果所有全局变量名称都使用”g\_”前缀，则可以方便地避免这一问题。
...
GCC和Clang支持标记 -Wshadow，如果变量被遮挡，则会生成警告。该标志有几个子选项（-Wshadow=global、-Wshadow=local和-Wshadow=compatible-local）。有关差异的解释，请参阅GCC文档。
