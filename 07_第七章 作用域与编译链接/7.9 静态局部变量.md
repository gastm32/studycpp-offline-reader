...
在该程序中，由于s\_value声明为静态，因此它是在程序启动时创建的。

可以在程序启动时，将静态局部变量初始化为0或constexpr值。

具有静态存储期的局部变量，如果未显式初始化，默认会被零初始化。对于非constexpr的初始值，静态局部变量在程序启动时先被零初始化，然后在首次执行到变量定义时才进行实际初始化。之后的调用将跳过该定义，不会进行重新初始化。

由于s\_value具有constexpr初始值1，因此将在程序启动时初始化s\_value。

当s\_value在函数末尾离开作用域时，它不会被销毁。每次调用函数incrementAndPrint() 时，s\_value的值保持之前留下的值。因此，该程序输出：
...
就像使用“g\_”为全局变量添加前缀一样，通常使用“s\_”为静态（静态存储期）局部变量添加前缀。
...
第一次调用此函数时，它返回0。第二次，它返回1。每次调用它时，它都会返回比上次调用它时高一的数字。可以将这些编号指定为对象的唯一ID。由于s\_itemID是局部变量，因此它不能被其他函数“篡改”。
...
getInteger() 不可重用，因为它具有内部状态（静态局部变量s\_isFirstCall），无法从外部重置。尽管程序在第一次编写时工作得很好，但静态局部变量阻止我们以后重用该函数。

实现getInteger的更好方法是将s\_isFirstCall作为参数传递。这允许调用者选择要打印的提示。
