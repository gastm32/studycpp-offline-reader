...
C++17引入了constexpr if语句，该语句要求条件必须是常量表达式。constexpr if语句的条件表达式会在编译时求值。
...
出于优化目的，现代编译器通常会把具有常量条件的非constexpr-if语句当作constexpr-if语句处理。然而，这并不意味着编译器一定会这样做。

遇到具有常量条件的非constexpr-if语句时，编译器可能会发出警告，建议您改用constexpr-if语句。这样可以确保条件在编译时求值（即使禁用了编译优化）。
