...
根据一组不同的值来测试变量或表达式是很常见的需求，因此C++提供了另一种称为switch语句的条件语句，专门用于处理这种情况。下面是与上面等价的程序：
...
与原始的if语句相比，switch语句的优点是只对表达式求值一次（因此效率更高）。此外，switch语句也能让读者更清楚地看到每种情况对应的处理语句。
...
我们使用switch关键字来开始一个switch语句，后面跟一对括号，其中包含要求值的条件表达式。表达式通常只是单个变量，但也可以是任何有效表达式。

一个限制是，条件必须求值为整数类型（请参见基本数据类型的介绍）或枚举类型（将在后续“无范围枚举”中介绍），或者可以转换为整数或枚举值。这里不能使用求值结果为浮点类型、字符串和大多数其他非整型的表达式。

条件表达式之后是一个代码块。在代码块内部，我们使用标签来定义所有要进行相等性测试的值。标签有两种。
...
在上面的程序中，x求值得到2。由于存在值为2的case标签，因此执行会跳转到匹配的case标签下方的语句。程序打印Two，然后执行return语句，函数返回给调用者。

case标签的数量没有实际限制，但switch中的所有case标签都必须是唯一的。也就是说，您不能这样做：
...
default标签是可选的，每个switch语句只能有一个default标签。按照惯例，default情况会放在switch块的最后。

**最佳实践**

将default标签放在switch块的最后。
...
在上面的示例中，x求值为5，但没有与5匹配的case标签，也没有default标签。因此，switch内的任何语句都不会执行。程序会从switch之后继续执行，并打印Hello。

---

## 使用break关键字

在上面的例子中，我们使用return语句来停止标签后续语句的执行。然而，这也会退出整个函数。

break语句（使用break关键字声明）告诉编译器：我们已经完成switch中的语句执行，需要执行switch之后的下一条语句。这允许我们在不退出整个函数的情况下退出switch语句。
