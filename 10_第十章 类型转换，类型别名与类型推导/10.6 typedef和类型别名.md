...
一旦定义，类型别名就可以在任何需要类型的地方使用。例如，我们可以创建一个使用类型别名作为类型的变量:
...
接下来，我们定义一个名为milesToDestination的变量，其类型为Distance。因为编译器知道Distance是类型别名，所以它会使用别名对应的类型，也就是double。因此，变量milesToDestination实际上会被编译为double类型的变量，并且在所有方面都将像double一样使用。

最后，我们打印milesToDestination的值，它打印为double。

**对于高级读者**

类型别名也可以模板化。我们将在后续介绍模板时讲解。
...
1. 使用以“\_t”后缀结尾的别名（“\_t”是“Type”的缩写）。标准库通常将此约定用于全局范围的类型名称（如size\_t和nullptr\_t）。

该约定继承自C，过去在定义自己的类型别名（有时还包括其他类型）时最流行，但在现代C++中已不受欢迎。请注意，POSIX为全局范围的类型名称保留了“\_t”后缀，因此使用此约定可能会导致POSIX系统上的类型命名冲突。

2. 使用以“\_type”后缀结尾的别名。某些标准库类型（如std::string）使用此约定来命名嵌套类型别名（例如，std::string::size\_type）。

但许多这样的嵌套类型别名根本不使用后缀（例如，std::string::iterator），因此这种用法是不一致的。

3. 使用不带后缀的别名。

在现代C++中的约定是：命名自己定义的类型别名（或任何其他类型）时，以大写字母开头，不使用后缀。大写字母有助于区分类型名称与变量和函数名称（通常以小写字母开头），并防止它们之间的命名冲突。
...
在这种情况下，Distance是类型，distance是参数名。C++区分大小写，所以这种写法可以正常工作。
...
尽管在概念上，我们希望Miles和Speed具有不同的含义，但两者都只是long类型的别名。这实际上意味着Miles、Speed和long都可以互换使用。事实上，当我们将Speed类型的值赋给Miles类型的变量时，编译器只会看到我们将long类型的值赋值给long类型变量，它不会发出警告。
...
由于向后兼容性的原因，typedef仍然存在于C++中，但在现代C++中，它在很大程度上已被类型别名取代。

typedef有几个语法问题。首先，很容易忘记是别名名称在前，还是要起别名的类型名称在前。下面哪个是正确的？
...
其次，typedef的语法在面对更复杂的类型时可能会变得难看。例如，下面是一个难以阅读的typedef，以及一个等效的（并且稍微更易阅读的）类型别名:

```
typedef int (*FcnType)(double, char); // FcnType 很难理解
using FcnType = int(*)(double, char); // FcnType 稍微容易阅读
```

在上面的typedef定义中，新类型的名称（FcnType）隐藏在定义的中间，而在类型别名中，新别名的名称和定义的其余部分由等号分隔。
...
由于char、short、int和long没有指示它们的大小，因此跨平台程序通常使用类型别名来定义包含类型大小（以位为单位）的别名。例如，int8\_t是8位有符号整数，int16\_t是16位有符号整数，int32\_t是32位有符号整数。以这种方式使用类型别名有助于防止错误，并使我们更清楚地了解对变量大小所做的假设。
...
在整数仅为2个字节的机器上，可以#define INT\_2\_BYTES（作为编译器/预处理器设置），这将使用上半部分定义的别名。在整数为4个字节的机器上，未定义INT\_2\_BYTES将导致使用下半部分的一组类型别名。通过这种方式，只要INT\_2\_BYTES定义正确，int8\_t将解析为1字节整数，int16\_t将解析为2字节整数，而int32\_t将解析为4字节整数（使用适合当前编译目标机器的char、short、int和long组合）。

固定宽度整数类型（如std::int16\_t和std::uint32\_t）和size\_t类型实际上只是各种基本类型的类型别名。
...
因为std::int8\_t通常是signed char的别名，所以变量x可能被定义为有符号字符。char类型将其值打印为ASCII字符，而不是整数值。
...
如果您不知道std::vector、std::pair或这些复杂的尖括号是什么，请不要担心。这里真正需要理解的一点是，类型别名允许您获取复杂的类型，并为它们提供一个更简单的名称，这使代码更容易阅读，也减少了输入负担。
...
TestScore作为返回类型，使函数返回测试分数这一意图变得更加明显。
...
虽然类型别名提供了一些好处，但它们也在代码中引入了另一个需要理解的标识符。如果这些额外成本不能被可读性或可理解性的提升所抵消，那么类型别名就会弊大于利。
