...
在我们希望变量及其初始值设定项具有相同类型的情况下，实际上提供了两次相同的类型信息。
...
在第一种情况下，由于5.0是double字面值，编译器会推导出变量d应该是double类型。在第二种情况下，表达式1+2产生一个int结果，因此变量i将是int类型。在第三种情况下，i先前被推导为int类型，因此x也将被推导为int。
...
类型推导不适用于没有初始值设定项或具有空初始值设定项的对象。当初始值设定项具有void类型（或任何其他不完整类型）时，它也不起作用。因此，以下内容无效：
...
int main()
{
    ...
    auto z{ foo() }; // z 不能是 void 类型, 所以本语句无效
...
如果希望推导出的类型带有const或constexpr，则必须自己提供const或constexpr。为此，只需将const或constexpr关键字与auto关键字一起使用：
...
如果希望从字符串字面值推导出的类型为std::string或std::string\_view，则需要使用s或sv字面值后缀：
...
如果add的返回类型从int变为double，或者gravity从int变为double，sum的类型也将从int变为double。

总的来说，现代共识是类型推导通常是安全的，并且可以弱化类型信息带来的干扰，让代码更具可读性，使代码逻辑更加突出。
