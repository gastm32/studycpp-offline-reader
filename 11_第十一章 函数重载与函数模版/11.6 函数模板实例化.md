...
上一课中，我们介绍了函数模板，并将普通的max()函数转换为max<T>函数模板：
...
函数模板本身并不是函数——它们的代码不会被直接编译或执行。相反，函数模板只有一项工作：生成函数。

要使用max<T>函数模板，可以使用以下语法进行函数调用：

```
max<实际类型>(参数一, 参数二); // 实际类型可能是 int、double 或其他类型
```
...
实例化函数的过程很简单：编译器本质上会克隆函数模板，并用我们指定的实际类型（int）替换模板类型（T）。
...
这与前面的示例类似，但这次我们的函数模板将用于生成两个函数：一次用int替换T，另一次用double替换T。在所有实例化之后，程序将如下所示：
...
在大多数情况下，我们希望用于实例化的实际类型与函数参数的类型匹配。例如：
...
请注意，第三个函数调用的语法看起来与普通函数调用完全相同！在大多数情况下，这是我们推荐的函数模板调用语法。
...
最后一点可能不明显。函数模板具有适用于多个类型的实现——也正因如此，它必须是泛型的。非模板函数只处理特定类型组合。它可以拥有比函数模板版本更优化或更专用于这些特定类型的实现。例如：

```
#include <iostream>

// 这个函数模板用于许多类型，因此它的实现比较简单
template <typename T>
void print(T x)
{
...
int main()
{
    ...
    print<>(true);     // 推导为 print<bool>(bool) (不考虑非模板函数) -- 打印 1
    std::cout << '\n';
...
当编译器尝试解析addOne(hello)时，它找不到与addOne(std::string)匹配的非模板函数，但它会找到我们的addOne(T)函数模板，并确定它可以从中生成addOne(std::string)函数。因此，编译器将生成并编译以下内容：
...
也许令人惊讶的是，由于C++在语法上允许将整数值添加到字符串字面值中（我们将在指针算术和下标中介绍这一点），因此上面的示例可以编译，并产生以下结果：
...
// 使用函数模板特化，告知编译器使用 addOne(const char*) 函数会产生编译错误
template <>
...
如果addOne是非模板函数，则该程序将正常工作：在main.cpp中，编译器只需要看到addOne的前向声明，链接器会将main.cpp中对addOne()的调用连接到add.cpp中的函数定义。

但由于addOne是模板，因此该程序无法工作，并且会出现链接器错误：
...
如果add.cpp实例化了这些函数，程序将可以正常编译并链接。但这样的解决方案很脆弱，应该避免：
...
这样，任何需要访问模板的文件都可以include相关头文件，并且模板定义会由预处理器复制到源文件中。然后，编译器就能够实例化所需的任何函数。
...
模板定义不受单定义规则中“每个程序只需要一个对象定义”这一限制的约束，因此将相同的模板定义包含在多个源文件中不是问题。并且从函数模板隐式实例化的函数是隐式内联的，因此可以在多个文件中定义它们，只要每个定义都相同。

模板本身不是内联的，因为内联的概念仅适用于变量和函数。

下面是另一个将函数模板放置在头文件中的示例，这样它可以被包含到多个源文件中：
...
```
#include "max.h" // 引入函数模板定义 max<T>(T, T)
#include <iostream>
...
在上面的示例中，main.cpp和foo.cpp都include了"max.h"，因此这两个文件中的代码都可以使用max<T>(T, T)函数模板。
