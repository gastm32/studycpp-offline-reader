...
非类型模板参数是具有固定类型的模板参数，它是作为模板参数传入的constexpr值的占位符。
...
对getSqrt(-5.0)的调用将在运行时assert失败，虽然这总比什么都没有要好。因为-5.0是一个字面值（并且隐式地是constexpr），如果我们可以使用static\_assert在编译时捕获这样的错误，会更好。然而，static\_assert需要常量表达式，而函数参数不能是constexpr…
...
template <double D> // 需要 C++20 才能使用浮点非类型模板参数
double getSqrt()
{
...
static\_assert条件为false，因此编译器将报错。
...
必须使用非类型模板参数来绕过函数参数不能是constexpr的限制，这并不理想。目前有不少相关提案正在评估中，以帮助解决这种情况。希望在未来的C++语言标准中看到更好的解决方案。
...
从C++17开始，可以使用auto让编译器从实际调用中推导非类型模板参数的类型：
