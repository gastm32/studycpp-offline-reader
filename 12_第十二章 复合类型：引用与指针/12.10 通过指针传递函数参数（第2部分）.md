...
在这个程序中，greet()函数有一个通过地址传递的参数，默认为nullptr。在main()中，调用该函数两次。第一次调用时，不知道用户是谁，因此在没有参数的情况下调用greet()。name参数默认为nullptr，greet函数打印“guest”。第二次调用时，现在有一个有效的用户，因此调用greet(&joe)。name参数接收joe的地址，并可以使用它来打印名称“Joe”。
...
// ptr被赋值但未被使用，[[maybe_unused]]让编译器不要警告
...
正如所看到的，更改指针参数ptr2存储的地址不会影响原始指针ptr所保存的地址（ptr仍然指向x）。当调用函数nullify()时，ptr2接收传入地址的副本（在本例中，是ptr持有的地址，即x的地址）。当函数更改ptr2指向的内容时，这仅影响ptr2持有的副本。
...
由于对指针的引用相当少见，因此很容易混淆语法（它是int\*&还是int&\*？）。好消息是，后一种写法会让编译器报错，因为不能有指向引用的指针（指针必须包含对象的地址，而引用不是对象）。

---

## 为什么不再推荐使用0或NULL表示空指针（可选）

0可以解释为整数，也可以解释为空指针。具体是哪一个可能是模棱两可的——在一些情况下，编译器可能会假设我们指的是其中一个，而我们的本意是另一个，程序可能会因此产生预期外的执行结果。

语言标准未规定预处理器宏NULL的具体定义。它可以定义为0、0L、((void\*)0) 或完全其他的东西。
...
由于nullptr可以与函数重载中的整数值区分开来，因此它必须具有不同的类型。那么nullptr是什么类型的呢？答案是，nullptr具有类型std::nullptr\_t（在头文件 cstddef 中定义）。std::nullptr\_t只能保存一个值：nullptr！虽然这看起来有点傻，但它在一种情况下是有用的。如果希望编写只接受nullptr参数的函数，可以将该参数类型设置为std::nullptr\_t。
...
在上面的示例中，函数调用print(nullptr)解析为函数print(std::nullptr\_t)而不是 print(int\*)，因为它不需要转换。

一种可能有点令人困惑的情况是，当ptr持有nullptr值时调用print(ptr)。记住，函数重载匹配的是类型，而不是值，并且ptr的类型是int\*。因此，print(int\*)将被匹配。在这种情况下，不会考虑print(std::nullptr\_t)，因为指针类型不会隐式转换为std::nullptr\_t。
...
既然您已经理解了通过引用、地址和值传递之间的基本区别，那么让我们来看看简化主义者的观点。：）
