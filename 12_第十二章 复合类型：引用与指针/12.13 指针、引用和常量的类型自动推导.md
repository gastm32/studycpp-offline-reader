...
通过在定义中添加const（或constexpr）限定符，可以（重新）应用const（或constexpr）：
...
在前面的示例中，我们讨论了ref1的情况。对于ref2，这类似于ref1的情况，只是重新应用了const限定符，因此推导出的类型是const std::string。
...
当我们将auto\*与指针类型初始值设定项一起使用时，为auto推导的类型不包括指针——在推导类型之后会重新应用指针。因此，对于上面的ptr2，auto的替代类型是std::string，然后重新应用指针。
...
第二，当存在const时，auto和auto\*的行为存在差异。下面介绍这一点。
...
当使用auto const或const auto时，我们是在说，“使推导出的类型成为const”。因此，在ptr1和ptr2的情况下，推导出的类型是std::string\*，然后应用const，使最终的类型为std::string\* const。这类似于const int和int const表示同一事物的方式。
...
在ptr7的情况下，const限定符会被应用两次，这是不允许的，并将导致编译错误。
