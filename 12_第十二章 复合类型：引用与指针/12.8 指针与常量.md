...
上面的代码段无法编译——不能将普通指针设置为指向const变量。这很有意义：const变量的值不能更改。如果允许程序员将普通指针指向常量，就能通过该指针修改常量，这违反了常量的定义。
...
就像const引用一样，指向常量的指针实际上可以指向非const变量。指向常量的指针会将所指向的值视为常量，而不管该地址处的对象最初是否定义为常量：
