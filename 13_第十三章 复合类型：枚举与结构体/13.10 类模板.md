...
在前面的函数模板课程中，我们介绍了一个挑战：必须为不同的参数类型创建单独的（重载）函数：
...
解决方案是创建一个函数模板，编译器可以使用该模板，为需要的任意类型集合实例化普通函数：
...
可以通过为Pair结构体赋予不同的名称（例如PairInt和PairDouble）来解决前两个问题。但是，必须记住它们的命名方案，并且为每个类型复制一组代码，这会造成严重的冗余。
...
就像函数模板一样，使用template开始类模板定义。先写template关键字。接下来，在尖括号（<>）内指定使用的所有模板类型。对于需要的每个模板类型，使用关键字typename（推荐）或class（不推荐），后跟模板类型的名称（例如T）。在这种情况下，由于两个成员变量是相同的类型，因此只需要一个模板类型。

接下来，像往常一样定义结构体，可以在任何需要模板化类型的地方使用模板类型（T），该类型稍后会被替换为真实类型。就是这样！我们完成了类模板定义。

在main中，可以使用所需的任何类型实例化Pair对象。首先，实例化一个类型为Pair<int>的对象。由于Pair<int>的类型定义尚不存在，编译器会使用类模板实例化名为Pair<int>的结构体类型定义，其中模板类型T的所有出现位置都会被类型int替换。
...
现在，让我们看看如何让max函数与类模板一起使用。由于编译器将Pair<int>和Pair<double>视为单独的类型，因此可以按参数类型区分重载函数：
...
但这不能解决冗余问题。我们真正想要的是一个可以接受任何Pair类型的函数。换句话说，需要一个接受Pair<T>类型参数的函数，其中T是模板类型参数。这意味着需要一个函数模板来完成这项工作！
...
max()函数模板非常简单。因为我们想传入Pair<T>，所以需要编译器理解T是什么。因此，需要一个模板类型参数 T 。然后，可以将T既用作返回类型，也用作Pair<T>的模板类型。
...
类模板可以让某些成员使用模板类型，其他成员使用普通类型。例如：
...
这与期望的完全一样：first 是模板类型 T ，second 总是int。
...
在某些情况下，可以编写与任何类型一起使用的函数模板。为此，只需要将模板类型参数直接作为函数参数的类型。
...
template <typename Pair> // 定义了一个模板类型参数 Pair (遮挡住了 struct Pair 的定义)
void print(Pair p)       // 这里指向模板类型参数 Pair, 而不是 struct Pair
{
    std::cout << '[' << p.first << ", " << p.second << ']';
}
```
...
由于使用数据对很常见，C++标准库包含一个名为std::pair的类模板（在<utility>头文件中），该模板与我们自己定义的类似。事实上，上述代码可以改用std::pair：
...
在本课中，我们编写了自己的Pair类来演示工作原理，但在实际代码中，更应该使用std::pair，而不是自己编写。
