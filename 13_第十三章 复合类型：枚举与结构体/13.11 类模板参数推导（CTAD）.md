...
从C++17开始，当从类模板实例化对象时，编译器可以根据对象初始值的类型推断模板参数类型（这称为类模板参数推导，简称CTAD）。例如：
...
int main()
{
    std::pair<int, int> p1{ 1, 2 }; // 显式声明 std::pair<int, int> (C++11)
...
只有不存在模板参数列表时，才会执行CTAD。因此，以下两项都是错误的：
...
int main()
{
    std::pair<> p1 { 1, 2 };    // error: 模板参数太少, 2个模板参数都缺少了
    std::pair<int> p2 { 3, 4 }; // error: 模板参数太少, 第二个模板参数缺少
...
在大多数情况下，CTAD可以直接使用。然而，在某些情况下，编译器可能需要一些额外帮助，才能理解如何正确推导模板参数。
...
int main()
{
    Pair<int, int> p1{ 1, 2 }; // ok: 显式指定模板参数
...
如果在C++17中编译它，可能会得到“类模板参数推导失败”“无法推导模板参数”或“没有可行的构造函数或推导指引”等错误。这是因为在C++17中，CTAD不知道如何推导聚合类模板的模板参数。为了解决这个问题，需要为编译器提供一个推导指引，告诉编译器如何推导给定类模板的模板参数。
...
int main()
{
    Pair<int, int> p1{ 1, 2 }; // 显式声明 std::pair<int, int> (C++11)
...
首先，使用与Pair类中相同的模板类型参数定义。因为如果推导指引要告诉编译器如何推导Pair<T，U>的类型，就必须先定义T和U是什么（模板类型）。其次，箭头右侧是希望编译器推导出的类型。在这种情况下，希望编译器能够为Pair<T，U>类型的对象推导模板参数。最后，箭头左侧告诉编译器要查找哪种声明。在这种情况下，告诉它查找一个名为Pair、具有两个参数的对象声明（一个是T类型，另一个是U类型）。也可以将其写为Pair(T t, U u)（其中t和u是参数名称，但由于不使用t和u，因此不需要为它们命名）。

把这些内容放在一起，就是告诉编译器：如果它看到一个具有两个参数的Pair声明（分别为T和U类型），就应该将类型推断为Pair<T，U>。

因此，当编译器看到 p2{1，2}; 时，它会理解为：“这是Pair的声明，并且有两个int类型的参数；根据推导指引，应该将其推导为Pair<int，int>”。
...
int main()
{
    Pair<int> p1{ 1, 2 }; // 显式声明 std::pair<int> (C++11)
...
std::pair（和其他标准库模板类型）附带预定义的推导指南，这就是为什么上面使用std::pair的示例在C++17中编译得很好。
...
struct Foo
{
    std::pair<int, int> p1{ 1, 2 }; // ok, 模板参数显式指定
