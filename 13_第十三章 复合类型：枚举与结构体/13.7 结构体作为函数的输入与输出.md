...
虽然传递3个单独的员工变量还不算太糟，但如果函数需要传递多个员工变量，独立传递每个变量就非常容易出错。此外，如果向员工添加新属性（例如名称），现在必须修改所有函数声明、定义和函数调用，以接受新的参数！
...
在上面的示例中，将整个Employee传递给printEmployee()（一次用于joe，一次用于frank）。
...
因为传递的是整个结构体对象（而不是单个成员变量），所以无论有多少个成员变量，都只需要一个参数。而且，如果决定向Employee结构体中添加新成员变量，也不必更改函数声明或函数调用！新成员变量将自动包含在内。
...
在这种情况下，将构造临时Point3d，将其复制回调用方，然后在表达式末尾销毁。这里比上面更加简洁（一行替代两行，并且不需要判断temp是否被多次使用）。
...
还要注意，在这种情况下，如果想要返回一个全零的结构体，可以使用空大括号来返回Point3d：
...
结构体很有用，类（C++和面向对象编程的核心）也直接构建在这里介绍的概念之上。充分理解结构体（特别是数据成员、成员选择和默认成员初始化）将使您更容易过渡到类。
