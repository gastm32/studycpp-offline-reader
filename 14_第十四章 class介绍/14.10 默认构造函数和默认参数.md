...
然而，正如之前讲解默认成员初始化时所述，值初始化对于聚合更安全。由于很难区分类类型是聚合还是非聚合，因此推荐对所有类类型使用值初始化。
...
在上面的示例中，实例化没有参数的foo，因此编译器会查找默认构造函数。它会找到两个，这是有歧义的，因此将导致编译错误。
...
该类没有用户声明的构造函数，因此编译器将为其生成隐式默认构造函数。该构造函数将用于实例化foo{}。
...
默认构造函数允许在用户不提供初始值时，创建非聚合类类型的对象。因此，只有当不需要用户提供初始值，并且类的默认值有意义时，才需要提供默认构造函数。
