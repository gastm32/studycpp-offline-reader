...
在前面，我们介绍了类型转换和隐式类型转换的概念：如果存在这样的转换方式，编译器会根据需要隐式地将一种类型的值转换为另一种类型的值。
...
在上面的示例中，printDouble函数有一个double参数，但传入了一个int类型的值。由于参数类型和传入值类型不匹配，编译器会查看是否可以隐式地将int值转换为double值。在这种情况下，使用数值转换规则，int值5会被转换为double值5.0。因为是按值传递，所以参数d将用该值进行拷贝初始化。
...
在此版本中，printFoo有一个Foo参数，但传入了一个int类型的数据。由于类型不匹配，编译器会尝试将int值5隐式转换为Foo对象，以便可以调用函数。
...
在这个版本中，将Foo类替换为Employee类。printEmployee有一个Employee参数，传入一个C样式的字符串文本。同时有一个转换构造函数：Employee(std::string\_view)。

您可能会惊讶地发现，这个版本不能编译。原因很简单：执行隐式转换时只能应用一次用户定义的转换，而这个示例需要两次。首先，必须将C样式的字符串文本转换为std::string\_view（使用std::string\_view转换构造函数），然后必须将std::string\_view转换为Employee（使用 Employee(std::string\_view) 转换构造函数）。
...
这是可行的，因为现在只需要一个用户定义的转换（从std::string\_view到Employee）。
...
这也可以工作，因为现在只需要一个用户定义的转换（从字符串文本到用于初始化Employee对象的std::string\_view）。将显式构造的Employee对象传递给函数不需要进行第二次转换。
...
当调用print(5) 时，Dollars(int) 转换构造函数会将5转换为Dollars对象。因此，该程序打印：
...
虽然这个例子很简单，但在更大、更复杂的程序中，编译器执行一些您没有预料到的隐式转换，并在运行时导致意外行为，这很容易让人感到惊讶。
...
为了解决这类问题，可以使用explicit关键字告诉编译器：构造函数不应被用作转换构造函数。
...
由于编译器不能再使用 Dollars(int) 作为转换构造函数，因此它无法找到将5转换为Dollars的方法。因此，它会生成编译错误。
...
这是允许的，因为仍然可以使用explicit构造函数来列表初始化对象。由于现在已经显式构造了一个Dollars，因此参数类型与传入数据类型匹配，不再需要转换！
...
现代最佳实践是，默认将任何接受单个参数的构造函数标记为explicit。这也包括那些有多个参数、但大多数或全部参数都有默认值的构造函数。

这将禁止编译器使用该构造函数进行隐式转换。

如果在特定情况下确实需要这样的转换，则可以很容易地使用列表初始化将隐式转换改写为显式定义。
...
例如，接收C样式字符串，参数类型为std::string\_view的构造函数不是explicit的，因为不太可能出现这样的情况，即我们不同意将C样式字符串视为std::string\_view。

相反，接收std::string\_view参数的std::string构造函数需要标记为explicit，因为虽然std::string值在语义上等同于std::string\_view值，但构造std::string的代价较高。

**最佳实践**

默认情况下，使任何接受单个参数的构造函数explicit。如果类型之间的隐式转换在语义上等效，并且是性能更好的转换，则可以考虑使构造函数非explicit。

不要标记拷贝或移动构造函数explicit，因为它们不执行转换。
