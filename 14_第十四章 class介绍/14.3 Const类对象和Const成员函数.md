...
在前面，我们知道基本数据类型（int、double、char等）的对象可以通过const关键字设置为常量。所有常量变量都必须在创建时初始化。
...
为了解决上述问题，需要使print()成为const成员函数，以保证它不会修改对象，也不会调用任何非const成员函数（因为它们可能会修改对象）。
...
由于const成员函数可以在常量和非常量对象上调用，因此如果成员函数不修改对象状态，则应将其设置为const。
...
在前面，我们介绍了通过常量引用而不是通过值传递类类型参数的优点。概括地说，按值传递类类型参数会导致生成类的副本（这很慢）——大多数情况下，不需要副本，对原始参数的引用就能很好地工作，并避免生成副本。
...
现在，在函数doSomething() 中，const date能够成功调用const成员函数print()。
