...
这个程序没有问题。但枚举类FruitType本来是为Fruit类配套使用的，却独立存在于类之外，这让我们不得不额外记住它们之间的关联。

---

## 嵌套类型（成员类型）

到目前为止，我们已经见过类类型的两种不同成员：数据成员和成员函数。在上面的例子中，Fruit类同时具有这两类成员。

类类型还支持另一种成员：嵌套类型（也称为成员类型）。要创建嵌套类型，只需在类内合适的访问说明符下定义类型。

下面是同一个程序，改写为使用Fruit类中定义的嵌套类型：
...
第四，类类型会作为其中声明名称的作用域，就像命名空间一样。因此，Type的完全限定名为Fruit::Type，而apple枚举元素的完全限定名为Fruit::apple。
...
最后，将枚举类型从 enum class 更改为 enum。由于类本身现在已经充当作用域，因此继续使用 enum class 有些多余。改为未限定作用域的枚举后，可以用Fruit::apple的形式访问枚举元素，而不必使用更长的 Fruit::Type::apple 。
...
我们之前讨论过Typedef和类型别名，它们在这里的作用相同。C++标准库中的类通常会使用嵌套的typedef。截至编写时，std::string定义了十个嵌套的typedef！

---

## 嵌套类和对外部类成员的访问

把其他类作为嵌套类型放进类中并不常见，但这是可行的。在C++中，嵌套类不能访问外部类的this指针，也不能直接访问外部类的成员。这是因为嵌套类可以独立于外部类进行实例化（在这种情况下，并不存在可访问的外部类实例和成员！）
...
有一种情况下嵌套类更常见。在标准库中，大多数迭代器类都被实现为容器的嵌套类，它们被设计用于遍历容器。例如，std::string::iterator被实现为std::string的嵌套类。我们将在未来的章节中介绍迭代器。
