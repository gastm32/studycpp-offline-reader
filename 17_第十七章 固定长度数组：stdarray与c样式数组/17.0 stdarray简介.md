...
* 容器提供对一组对象的存储管理能力
* 数组会将内部对象连续存放在内存中，因此可以通过元素在数组中的位置快速访问
* C++有三种不同的常用数组类型: std::vector，std::array，C样式的数组

数组又分为两类:

* 固定大小的数组。在数组创建时，必须指定一个长度，之后也不能再修改，std::array和 C样式数组属于这一类
* 动态数组，可以动态地调整长度，std::vector 属于这一类

---

## 那么，为什么不对所有内容都使用动态数组呢？

动态数组功能强大且方便，但和许多工具一样，它们在带来便利的同时也存在一些权衡。
...
## 使用std::to\_array（C++20）省略数组长度

然而，TAD（模板参数推导，用于函数模板解析）支持部分省略模板参数。从C++20以后，可以通过使用std::to\_array helper函数来省略std::array的数组长度:
...
不幸的是，使用std::to\_array比直接创建std::array代价更高，因为它会先创建一个临时std::array，然后再用该数组复制并初始化目标std::array。由于这个原因，std::to\_array应该仅在无法从初始值设定项有效确定类型时使用，并且应避免在需要多次创建数组的场景中使用（例如循环中）。
