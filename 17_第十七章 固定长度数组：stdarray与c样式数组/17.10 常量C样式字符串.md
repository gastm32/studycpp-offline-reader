...
这是两个不同的字符串变量，具有相同的值。由于这些字符串是常量，编译器可以选择在内存中只存放一份“Alex”，并让name1和name2都指向相同的地址。
...
它为什么这样做？首先，它假设&c（类型为char\*）是C样式字符串。因此它打印了“Q”，然后继续。接下来是一堆垃圾。最终，它遇到了一些内存，其中包含一个0值，它将该值解释为空终止符，因此它停止了。根据变量c之后内存中的内容，您看到的可能会有所不同。

这种情况在实际代码中不太可能发生（因为您不太可能真的想打印字符变量的内存地址），但它说明了幕后发生了什么，以及程序如何在无意中偏离预期。

如果您确实想打印字符指针的地址，请将其static\_cast为类型const void\*：
...
## 优先使用std::string\_view而不是C样式的字符串常量

在现代C++中，几乎没有理由使用C样式的字符串常量。相反，应该优先使用constexpr std::string\_view对象，它们往往同样快，并且行为更一致。
