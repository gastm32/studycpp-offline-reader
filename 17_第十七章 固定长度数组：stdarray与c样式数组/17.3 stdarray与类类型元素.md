...
然而，初始化结构体或类类型的std::array常常会让新程序员困惑，因此我们将花一节课专门讨论这个主题。
...
在上面的示例中，CTAD会将std::array的类型推断为std::array<House，3>。随后提供的3个House对象会作为初始值设定项。
...
我们还没有介绍C样式数组，但出于本课的目的，您只需要知道的是“T implementation\_defined\_name[N];”代表的是 T类型的N个元素的固定大小数组。
