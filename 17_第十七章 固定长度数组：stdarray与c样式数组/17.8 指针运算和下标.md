...
给定某个指针ptr，ptr+1返回内存中下一个对象的地址（基于所指向的类型）。因此，如果ptr是int\*，且int是4个字节，ptr+1将返回ptr之后4个字节处的内存地址，ptr+2将返回ptr之后8个字节处的内存地址。
...
在这种情况下，每个内存地址都比前一个少4个字节。

将自增（++）和自减（–）操作符应用于指针，会分别执行与指针加法和指针减法相同的操作，但它们会实际修改指针持有的地址。
...
指针运算返回下一个/前一个对象的地址（基于所指向的类型），而不是简单的下一个或前一个字节地址。

**警告**

从技术上讲，以上是未定义的行为。根据C++标准，指针运算仅在指针和结果位于同一数组（或超过末尾的数组）内时有效。然而，现代C++实现通常不强制执行这一点，并且通常不会禁止您在数组之外使用指针运算。
...
您会注意到，arr保存的是地址00AFFD80，（arr+1）是4个字节之后的地址，而（arr+2）是8个字节之后的地址。可以通过这些地址获得对应的元素。
...
当然，您也会注意到，如果我们不能假设ptr[1]总是索引为1的元素，那么程序会更加混乱。因此，建议仅在进行相对定位时使用指针运算。
...
指针运算最常见的用途之一，是在没有显式索引的情况下迭代C样式数组。下面的示例说明了如何完成此操作:
...
在上面的例子中，我们从begin所指向的元素（在本例中是数组的元素0）开始遍历。begin != end 时，循环体执行。在循环中，通过\*begin访问当前元素，这只是一次指针解引用。在循环体之后，我们执行++begin，它使用指针运算让begin指向下一个元素。只要begin != end 仍为true，循环体就会再次执行。这个过程会一直持续到 begin != end 为false。
...
对于指向C样式数组元素的指针，只要生成的地址是有效数组元素的地址，或者是最后一个元素之后的地址，指针运算就有效。如果指针运算导致地址超出这些界限，则这是未定义的行为（即使结果未解引用）。

---

## 使用指针运算实现C样式数组上的基本范围for循环
...
请注意，这与我们在上一节中编写的示例非常相似！唯一的区别是我们将\*\_\_begin分配给e并使用e，而不是直接使用\*\_\_begin！
