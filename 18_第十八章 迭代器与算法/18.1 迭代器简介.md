...
在编程中，迭代数组（或其他数据结构）中的数据是非常常见的事情。到目前为止，我们已经介绍了许多不同的方法:使用循环和索引（对于for循环和while循环）、使用指针和指针运算，以及使用基于范围的循环:
...
int main()
{
    ...
    // 使用指针的for循环 (注: ptr 不能是 const, 因为需要进行累加)
...
可以使用索引来访问元素，但这种方式仅在容器（例如数组）提供对元素的直接访问时有效（数组提供这种访问，但其他类型的容器（如列表）不提供这种访问）。

使用指针和指针运算进行循环是冗长的，并且可能会使不了解指针运算规则的读者感到困惑。指针运算也仅在元素在内存中连续时有效（这对于数组是正确的，但对于其他类型的容器（如列表、树和map）并不成立）。
...
最简单的迭代器类型是指针，它可以（通过指针运算）用于顺序存储在内存中的数据。让我们使用指针和指针运算重新访问一个简单的数组遍历:
...
在上面，我们定义了两个变量:begin（指向容器的开始）和end（标记终点）。对于数组，结束标记通常是内存中最后一个元素之后的位置。

然后指针在begin和end之间迭代，可以通过解引用指针来访问当前元素。
...
int main()
{
    ...
    // 使用 std::begin 和 std::end 获取开始和结束.
...
在之前的循环示例中，我们注意到使用 运算符< 比 运算符!= 更好:


    for (index = 0; index < length; ++index)


对于迭代器，通常使用 运算符!= 来测试迭代器是否已到达end元素，如下所示:
...
所有同时具有begin()和end()成员函数的类型，或者可以使用std::begin与std::end处理的类型，都可以使用基于范围的for循环。
...
在幕后，基于范围的for循环会调用要迭代类型的begin()和end()。array具有begin和end成员函数，因此我们可以在基于范围的循环中使用它。C样式固定数组可以与std::begin和std::end函数一起使用，因此我们也可以使用基于范围的循环来遍历它们。然而，动态C样式数组（或退化的C样式数组）无法工作，因为它们没有std::end函数（因为类型信息不包含数组的长度）。
...
一些修改容器的操作（例如将元素添加到std::vector）可能会产生副作用，导致容器中的元素更改地址。当这种情况发生时，这些元素的现有迭代器将失效。好的C++参考文档应该标注哪些容器操作可能或一定会使迭代器失效。例如，请参阅cppreference上std::vector的“迭代器无效”部分。
...
通过为无效迭代器重新赋予有效迭代器（例如，begin()、end()或其他返回迭代器的函数），可以让迭代器重新变得有效。

erase函数会返回被删除元素的下一个元素的迭代器（如果最后一个元素被删除，则返回end()）。因此，我们可以如下修复上述代码:
