...
标识符foo是函数的名称。但函数是什么类型的呢？函数有自己的左值函数类型——在本例中，是一种不带参数并返回整数的函数类型。就像变量一样，函数也存在于内存中的指定地址。

调用函数时（通过“()”运算符），程序会跳转到被调用函数的地址并开始执行：
...
运算符«不知道如何输出函数指针（因为可能有无限多种函数指针）。标准规定，在这种情况下，foo应该转换为bool（操作符«知道如何打印bool）。由于foo的函数指针是非空指针，因此它的计算结果应该始终为true。因此，应打印：

```
1
```

就像可以声明指向普通变量的非常量指针一样，也可以声明指向函数的非常量指针。在本课的其余部分中，我们将研究这些函数指针及其用法。函数指针是一个相当高级的主题，如果您只想学习C++基础知识，可以放心跳过本课剩余部分。
...
出于优先级原因，\*fcnPtr周围的括号是必要的，因为“int\* fcnPtr();”会被解释为名为fcnPtr的函数的前向声明，该函数不接受参数，并返回指向整数的指针。
...
2. https://web.archive.org/web/20110818081319/http://ieng9.ucsd.edu/~cs30x/rt\_lt.rule.html

---

## 将函数分配给函数指针

函数指针可以用函数初始化（也可以为非const函数指针重新分配函数）。与变量指针一样，我们也可以使用&foo来获得指向foo的函数指针。
...
这会尝试将函数goo()调用的返回值（类型为int）分配给fcnPtr（它需要类型为int(\*)()的值），这不是我们想要的。我们希望为fcnPtr分配函数goo的地址，而不是函数goo()的返回值。因此不需要括号。
...
与基本类型不同，如果需要，C++会隐式地将函数转换为函数指针（因此不需要使用操作符&来获取函数地址）。然而，函数指针不能转换为void指针，反之亦然（尽管某些编译器，如Visual Studio，可能允许这样做）。
...
正如您所看到的，隐式解引用方法看起来就像普通函数调用——这也符合预期，因为普通函数名本来就可以转换为函数指针！然而，一些较旧的编译器不支持隐式解引用函数指针，但所有现代编译器都应该支持。

还要注意，因为函数指针可以设置为nullptr，所以最好在调用函数指针之前使用断言或条件判断来检查函数指针是否为nullptr。就像普通指针一样，解引用空函数指针会导致未定义的行为。
...
这意味着我们可以使用函数指针来消除由默认参数导致的函数调用歧义。在下面的示例中，我们展示了两种方法：
...
函数指针最有用的用途之一，是将函数作为参数传递给另一个函数。用作另一个函数参数的函数有时称为回调函数。

考虑这样一种情况：您正在编写一个函数来执行任务（例如对数组进行排序），但希望用户能够定义该任务中某个特定部分的执行方式（例如数组按升序还是降序排序）。让我们更仔细地看一下排序这个具体问题，它也可以推广到其他类似问题。
...
现在，为了让调用方决定如何排序，而不是使用我们自己硬编码的比较函数，我们将允许调用方提供自己的排序函数！这是通过函数指针完成的。
...
因此，我们将允许调用者向排序程序传递一个指向所需比较函数的指针作为第三个参数，然后使用调用者提供的函数进行比较。
...
如您所见，在这种上下文中使用函数指针是一种很好的方法，它允许调用方将自己的函数“挂接”到您已经编写并测试过的代码中，从而促进代码重用！以前，如果希望按降序对一个数组排序，并按升序对另一个数组排序，就需要多个版本的排序函数。现在，只需要一个版本，就可以按调用方希望的任何方式排序！
...
这仅适用于函数参数，因此用途有限。在非函数参数的位置，后者会被解释为前向声明：
...
如果您要允许调用者将函数作为参数传入，那么通常也应该为调用者提供一些标准函数，方便他们使用。例如，在上面的选择排序示例中，提供ascending()和descending()函数以及selectionSort()函数，会让调用者的工作更轻松，因为他们不必每次想使用它们时都重写ascending()或descending()。
...
在这种情况下，只要用户正常调用selectionSort（而不是通过函数指针调用），comparisonFcn参数将默认为ascending。您需要确保在此之前已经声明ascending函数。

---

## 使用类型别名使函数指针更漂亮

让我们面对现实——函数指针的语法很难看。然而，类型别名可以让函数指针看起来更像普通变量：
...
定义和存储函数指针的另一种方法是使用std::function，它是标准库<functional>头文件的一部分。要使用这种方法定义函数指针，请声明一个std::function对象，如下所示：
...
还要注意，std::function只允许通过隐式解引用（例如，fcnPtr()）调用函数，而不允许通过显式解引用调用函数（例如，(\*fcnPtr)()）。
...
就像auto关键字可以用于推断普通变量的类型一样，auto关键字也可以推断函数指针的类型。
...
函数指针主要在希望将函数存储在数组（或其他结构）中，或者需要将函数传递给另一个函数时有用。由于声明函数指针的原始语法难看且容易出错，因此建议使用std::function。在函数指针类型只使用一次的地方（例如单个参数或返回值），可以直接使用std::function。在多次使用函数指针类型的地方，最好选择std::function的类型别名（以避免重复书写）。
