...
在函数模板实例化中，我们讨论了编译器如何使用函数模板实例化函数，然后编译这些函数。我们还注意到，如果函数模板中的代码试图执行实际类型不支持的操作（例如将整数值1添加到std::string），这些函数可能无法通过编译。

在本课中，我们将看几个示例：实例化后的函数因为实际类类型不支持某些运算符而无法编译。然后，我们会展示如何定义这些运算符，使实例化函数能够通过编译。
...
然后它会尝试编译这个函数。看到这里的问题了吗？当x和y是Cents类型时，C++不知道如何计算"x < y"！因此，这会产生编译错误。

要解决此问题，只需要为我们希望与max一起使用的类重载运算符<：
...
让我们再举一个函数模板的例子，它会因为缺少重载运算符而无法工作。
...
此错误实际上是由调用average（const Cents\*，int）时创建的函数模板实例引起的。请记住，当我们调用模板函数时，编译器会“生成”该函数的一个副本，其中模板类型参数（占位符类型）已被函数调用中的实际类型替换。下面是T为Cents时average（）的函数模板实例：
...
在这种情况下，sum是一个Cents对象，但我们没有为Cents对象定义运算符+=！我们需要定义这个函数，average（）才能使用Cents。可以看到average（）也使用了运算符/=，因此也要定义它：
...
最后，我们的代码可以编译并运行！结果如下：

```
11 cents
```

请注意，我们根本不需要修改average（）本身，就能让它处理Cents类型的对象。我们只需要定义average（）在Cents类上所需的运算符，其余工作由编译器处理！
