...
可以更改派生类中继承而来的成员的访问说明符。只需在派生类中对应的访问说明符下，使用「using」声明基类的对应成员即可。
...
由于Base::printValue()已声明为protected，因此只能由Base或其派生类调用。外部无法访问它。

让我们定义一个Derived类，将printValue()的访问说明符更改为public：
...
在C++中，除了修改源代码外，无法从基类中删除或限制功能。然而，在派生类中，可以隐藏基类中存在的功能，使其不能通过派生类访问。这可以通过简单地更改相关访问说明符来完成。
...
这允许我们使用一个设计不佳的基类，并将其封装在派生类中。可以public继承Base的成员，然后修改其访问说明符。或者，也可以private继承Base，这会导致Base的所有成员首先被private继承。

然而，值得注意的是，虽然m\_value在Derived类中是私有的，但它在基类中仍然是public的。因此，通过强制转换为Base&并直接访问成员，仍然可以破坏Derived中m\_value的封装。

出于相同原因，如果基类具有public virtual函数，而派生类将其访问说明符更改为private，则外部仍然可以通过将派生对象强制转换为Base&并调用虚函数，来访问private的派生函数。编译器允许这样做，因为该函数在Base中是public的。然而，由于对象实际上是派生类对象，因此虚函数解析会解析（并调用）该函数的（私有）派生版本。访问控制不会在运行时强制执行。
...
也许令人惊讶的是，对于基类中的一组重载函数，没有办法只更改单个重载的访问说明符。只能全部更改：
...
您还可以将派生类中的成员函数标记为删除，以确保它们不能通过派生对象调用：
...
在上面的示例中，我们将getValue()函数标记为删除。这意味着当我们试图调用该函数的派生类版本时，编译器将报错。请注意，getValue()的基类版本仍然可以访问。我们可以使用下面两种方法之一调用Base版本：
...
如果使用转换方法，则推荐转换为Base&而不是Base，以避免发生实际复制。
